...
- **Vila 360º (Shining in the Darkness style):** Pedralume agora possui um **panorama contínuo** na praça central. Pressionando `Esquerda` ou `Direita`, a câmera gira 360 graus, revelando os locais (Forja, Loja, Catacumbas, etc.) com letreiros luminosos indicando os *hotspots*. Pressione `W` para entrar fisicamente em uma loja (onde a arte interna 2D sem personagens de fundo destaca os sprites dos NPCs) e `S` para retornar imediatamente à praça. O Automapa é desativado na vila, reforçando seu caráter de Safe-Zone.
